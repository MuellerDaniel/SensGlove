...
\subsubsection{Evaluation, Discussion and Comparison to Leap Motion and Video Data}
	...
	\item discuss the model configurations (also similar to simulated data):	
		...
		\item comparing \grqq 11\grqq with \grqq 14\grqq
				%\begin{itemize}	
				%\item results for 11 are much worse than multiple sensors (160210\_set2)
				%\item estimation is faster than one - four (obvious and stated beforehand...)
				%\item e.g. four-one better than one-one 160210\_set4 (beginning of fist)
				%\item \todo{set for showing, that 12 is better than 11}
				%\item \todo{set for showing influence 12 vs 14}
				%\end{itemize}
			
		\item comparing results/difference for using cyl and dip
			%\begin{itemize}
			%\item dip is much faster than cyl (around 7 times faster!) (obvious, since stated before...)
			%\item regarding accuracy, no clear statement possible...
			%\item sometimes cyl is better, sometimes dip
			%\item \todo{sets for showing this: }
			%\end{itemize}
	...
	\item state your observations/drawbacks from Leap \todo{Don't I destroy now all statements from beforehand, by saying that the Leap is not perfect?}
				\begin{itemize}
