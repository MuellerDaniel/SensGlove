...
\end{tabular}
\caption[The parameters for the estimation results, showing the smallest difference to the Leap Motion]
...
\todo{Adjust the table! Do it like in the excel sheet! Import it as a csv file TOO LONG!!!}}
\label{tab:estSet}
\end{table}
...
However the set showing the smallest deviation over all recorded sets uses only 2 sensor units for the estimation (cyl\_12). Set 4 shows a mean and standard deviation of $ 0.467 \si{radian} \pm 0.027 $. By regarding at the overall quality of the states, obtained of the magnetic estimation versus the states from the Leap motion system, one can note that the errors are pretty high. The mean over all sets is $ 0.602 \si{\radian} \pm 0.00 $, which corresponds to \SI{34.5}{\degree}. As mentioned beforehand this value has to be handled with care, since each dataset shows different size and motions. The time needed for the estimation of the states is also documented. Here, almost no differences to the results, obtained in \ref{subsubsec:simEval} are found. The timing behaviour of the slowest set was evaluated to be \SI{0.092}{\second} and is observed by a method using four sensor readings. This value is in fact higher, than the system frequency, but the results are expected not to degrade, since at maximum one data set gets neglected. Regarding the realtime capability, the respective worst estimation frequency would be \SI{10}{\Hz}.
