...
zSensor = 0.0
s1 = [0., 0.0, zSensor]
s2 = [s1[0], -0.0203, zSensor]     # sensor beneath middle
s3 = [s1[0], -0.0203*2, zSensor]    # sensor beneath ring
s4 = [s1[0], -0.0203*3, zSensor]    # sensor beneath pinky
...
#jointInd = [xJoint, s1[1], -0.026]
#jointMid = [xJoint, s2[1], -0.022]
#jointRin = [xJoint, s3[1], -0.023]

a = np.arange(-0.01,0.01,0.002)
...
for i in a:         # virtually moving the rack from right to left on the hand (in y-direction)
    jointMid[cnt] = [xJoint+i, s2[1], -0.022]
    jointRin[cnt] = [xJoint+i, s3[1], -0.023]    
