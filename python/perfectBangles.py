...
sensList_cyl = [h.sInd_rad, h.sMid_rad, h.sRin_rad, h.sPin_rad]
#sensList_cyl = [h.sPin_rad]
fingList_cyl = [h.phalInd, h.phalMid, h.phalRin, h.phalPin]
#fingList_cyl = [h.phalPin]
jointList_cyl = [h.jointInd_rad, h.jointMid_rad, h.jointRin_rad, h.jointPin_rad]
...
b_cyl = np.zeros((len(t), 2*len(sensList_cyl)))
b_dip = np.zeros((len(t), 3*len(sensList_dip)))    

cnt = 0
for i in range(len(t)):
    b_cyl[i] = modC.cy.angToBm_cyl_cy(angles_cyl[i],fingList_cyl,sensList_cyl,jointList_cyl)    # simulating cylindrical model
